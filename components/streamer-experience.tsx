  ...
  return (
    <main className="page-shell">
      ...
      <section className="streamer-hero">
        ...
        <div><span className="kicker">STREAMER ARCHIVE</span><h1>{streamer?.channelName ?? "채널 기록"}</h1><p>수집된 방송의 채팅 반응 타임라인입니다.</p></div>
      ...
      <section className="content-section compact">
        ...
        {broadcasts.data?.data.length ? <VirtualBroadcastList broadcasts={broadcasts.data.data} /> : (
          <div className="empty-card"><Radio /><div><h3>아직 기록된 방송이 없어요.</h3><p>방송이 시작되면 채팅 수집을 자동으로 시작합니다.</p></div></div>
        )}
      </section>
    </main>
  );
